...
def merge_sort_step(data, drawData, timeTick):
    merge_sort_alg_step(data, 0, len(data)-1, drawData, timeTick)


def merge_sort_alg_step(data, left, right, drawData, timeTick):
    if(left < right):
        middle = (left + right) // 2
        merge_sort_alg_step(data, left, middle, drawData, timeTick)
        merge_sort_alg_step(data, middle + 1, right, drawData, timeTick)
        merge_step(data, left, middle, right, drawData, timeTick)


def merge_step(data, left, middle, right, drawData, timeTick):
    drawData(data, getColorArray(len(data), left, middle, right))
    time.sleep(timeTick)
