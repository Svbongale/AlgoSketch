...
# swap pivot with border values
    drawData(data, getColorArray_step(len(data), head, tail, border, tail, True))
    time.sleep(timeTick)
    data[border], data[tail] = data[tail], data[border]
    running  = False
    return border        
...
def quick_sort_step(data, head, tail, drawData, timeTick):
    global running
    if running: 
        if head < tail:
            partitionIndx = partition_step(data, head, tail, drawData, timeTick)
        # Left partition
            running = False
            if running:
                quick_sort_step(data, head, partitionIndx-1, drawData, timeTick)
        # Right partition
                quick_sort_step(data, partitionIndx+1, tail, drawData, timeTick)
